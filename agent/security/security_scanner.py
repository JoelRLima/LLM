...
from agent.security.security_patterns import lookup
from agent.skills.security_symbols import get_pattern_id_map
...
#
# Antes, esta era uma cópia mantida manualmente em sincronia com os
# conjuntos de símbolos de code_analyzer.py (achado crítico 1.7). Agora é
# derivada em tempo de importação a partir do registro único
# `code_analyzer.SECURITY_SYMBOL_REGISTRY`, via get_pattern_id_map().
# Símbolos como "hardcoded_secret", "sql_string_format" e
# "flask_debug_true" não vêm do code_analyzer (são gerados por outras
# fontes, ex.: grep) e continuam declarados aqui como extensão.
_TYPE_TO_PATTERN = {
    **get_pattern_id_map(),
...
def _map_to_pattern_id(symbol: str) -> str:
    ...
    return str(_TYPE_TO_PATTERN.get(symbol, "PY999"))  # PY999 = desconhecido


def consolidate(code_analyzer_result: dict[str, Any], grep_results: list[Any] | None = None) -> List[Finding]:
    """
