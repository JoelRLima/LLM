...
class OperationalEvidence:
    ...
    surviving_files: set[str] = field(default_factory=set)
...
def collect_operational_evidence(state: Any) -> OperationalEvidence:
    ...
    task_rollback = bool(getattr(state, "_task_rollback_occurred", False))
    facts.rollback_occurred = task_rollback
    task_rollback_succeeded = getattr(state, "_task_rollback_succeeded", None)
    if task_rollback and type(task_rollback_succeeded) is bool:
        facts.rollback_succeeded = task_rollback_succeeded
    ...
    # Invocation artifacts describe the state immediately after that
    # invocation. Once a task rollback is confirmed, they no longer describe
    # mutations surviving the task boundary.
    if facts.rollback_succeeded is True:
        facts.surviving_files.clear()
    return facts
...
def _add_history_entry(
    ...
    artifact = project_mutation_evidence(result)
    facts.files.update(artifact.mutated_files)
    facts.files.update(artifact.surviving_files)
    facts.surviving_files.update(artifact.surviving_files)
    ...
    if artifact.rollback_occurred and facts.rollback_succeeded is None:
        facts.rollback_succeeded = artifact.final_state == "restored"
...
def _add_incident(facts: OperationalEvidence, incident: Mapping[str, Any]) -> None:
    ...
    if incident.get("rollback_occurred") is True:
        ...
        if facts.rollback_succeeded is None:
            facts.rollback_succeeded = True
