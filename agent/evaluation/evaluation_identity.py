"""Immutable campaign candidate, fixture, and model/config identities.

The campaign identity exposes a broad source hash and a planned model label,
alongside a semantic manifest and a canonical non-secret model/config
fingerprint. Documentation can therefore change independently while any
execution, evaluation, fixture, or provider change invalidates a campaign
epoch.
"""
...
from agent.evaluation.model_identity import (
    DEFAULT_PROFILE,
    fake_model_identity,
...
def source_fingerprint(repo_root: str | Path) -> str:
    """Compatibility hash used by campaign reports."""
