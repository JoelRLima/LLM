...
def _measurement_summary(measurement: dict[str, object]) -> dict[str, object]:
    allowed = {
        ...
        "terminal_outcome", "error", "output_chars", "truncated", "tool_history_count", "model_calls", "status",
    }
    ...
    for key in allowed:
        ...
        if key in {"error", "task_id", "terminal_outcome", "status"}:
            ...
            result[key] = [
                {"invocation_id": _bounded(item.get("invocation_id"), 200), "outcome": _bounded(item.get("outcome"), 100)}
