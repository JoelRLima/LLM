"""Versioned H-series scenario definitions and bounded evidence contracts.
...
H_SERIES_VERSION = "H-SERIES-V1.5"
...
class RepetitionPolicy:
    """Bounded repetition policy for the H-series campaign."""
    ...
    def __post_init__(self) -> None:
        if self.initial_repetitions < 1 or self.h2_repetitions < 1:
            raise EvidenceContractError("repetitions must be positive")
        if self.h2_repetitions > self.maximum_repetitions:
            raise EvidenceContractError("H2 repetitions exceed campaign maximum")
    ...
    def initial_decision(self, pass_count: int, valid_count: int) -> str:
        ...
        if valid_count != self.initial_repetitions:
            raise EvidenceContractError("initial repetition decision requires exactly three valid samples")
...
class HSeriesArm:
    ...
    def __post_init__(self) -> None:
        if self.approval_mode not in {"explicit", "required"}:
            raise EvidenceContractError("invalid H-series approval mode")
    ...
    def to_capability_scenario(self, h_id: str) -> CapabilityScenario:
        ...
        return CapabilityScenario(
            ...
            capability=f"h-series/{h_id.lower()}",
...
class HSeriesScenario:
    ...
    def __post_init__(self) -> None:
        if not re.fullmatch(r"H(?:[1-9]|1[0-9])", self.h_id):
            raise EvidenceContractError(f"invalid H-series id: {self.h_id}")
        if not self.arms:
            raise EvidenceContractError(f"{self.h_id} has no fixture arms")
        if len({arm.arm_id for arm in self.arms}) != len(self.arms):
            raise EvidenceContractError(f"{self.h_id} has duplicate arm ids")
        if self.required_repetitions < 1:
            raise EvidenceContractError(f"{self.h_id} has invalid repetition count")
...
class HRunEvidence:
    ...
    def __post_init__(self) -> None:
        if not self.scenario_id.strip() or self.repetition < 1:
            raise EvidenceContractError("run evidence requires scenario id and positive repetition")
        if not self.epoch.strip() or not self.candidate_identity.strip():
            raise EvidenceContractError("run evidence requires epoch and candidate identity")
        if not isinstance(self.evidence_level, EvidenceLevel):
            raise EvidenceContractError("invalid evidence level")
        if not isinstance(self.causal_classification, CausalFailureClass):
            raise EvidenceContractError("invalid causal classification")
...
def _load_h_series() -> tuple[HSeriesScenario, ...]:
    from agent.evaluation.h_series_scenarios import H_SERIES
...
def validate_h_series(scenarios: Sequence[HSeriesScenario] | None = None) -> None:
    ...
    if ids != expected:
        raise EvidenceContractError(f"H-series must contain exactly H1-H19, got {ids!r}")
    if any(
        scenario.required_repetitions != (5 if scenario.h_id == "H2" else 3)
        for scenario in selected
    ):
        raise EvidenceContractError("H-series repetition policy is inconsistent")
