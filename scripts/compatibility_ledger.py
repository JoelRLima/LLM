...
LEDGER = (
    ...
    _edge("W16-C01", "agent/llm/model_compatibility.py", "ModelCompatibility", RECLASSIFY_CANONICAL, "typed model compatibility policy", "model profile and request geometry", "none", "the typed policy owner expresses provider compatibility configuration, not a source compatibility facade", "no retirement; canonical typed policy owner"),
    _edge("W16-C02", "agent/llm/model_profile_compat.py", "compatibility_from_raw", RECLASSIFY_CANONICAL, "typed model compatibility policy", "profile ingress", "raw profile configuration", "the parser is the canonical strict configuration boundary for the typed policy", "no retirement; canonical profile ingress"),
    _edge("W16-C03", "agent/runtime/config_schema.py", "_validate_compatibility", RECLASSIFY_CANONICAL, "configuration schema validation", "profile configuration boundary", "persisted profile mappings", "the validator enforces the closed typed policy vocabulary at configuration ingress", "no retirement; canonical schema boundary"),
    _edge("W16-C04", "agent/runtime/config_schema.py", "COMPATIBILITY_FIELDS", RECLASSIFY_CANONICAL, "configuration schema validation", "profile configuration boundary", "persisted profile mappings", "the field map is the closed schema declaration for the typed policy", "no retirement; canonical schema declaration"),
    ...
    _edge("W19-S02-C01", "agent/evaluation/comparison.py", "_validate_groups_compatibility", RECLASSIFY_CANONICAL, "agent.evaluation.comparison", "receipt group comparison callers", "W19 evaluation receipts", "compatibility describes semantic identity comparability here, not a source/API compatibility facade", "no retirement; canonical comparison guard"),
    _edge("W19-S05-C01", "agent/runtime/storage_migration.py", "_compatible_previous_receipt", RECLASSIFY_CANONICAL, "agent.runtime.storage_migration", "migrate_plan / migration retry path", "W19 storage migration receipt", "compatible describes semantic comparability of a receipt already validated against migration id/source profile during retry; it is not a source/API facade or physical legacy fallback", "no retirement; canonical migration retry-safety guard"),

    # W19 Domain 07 cleanup edges: retired source facades and bounded retained
    # compatibility boundaries.  These entries are source-only and therefore
    # intentionally have no durable consumers.
    _edge("W19-S07-R01", "agent/llm/router.py", "<module>", REMOVE, "agent.routing.persona.*", "repository routing callers migrated to the current persona owner", "none", "the historical router mixed production routing semantics and was replaced by the injected canonical persona router", "already absent; no replacement facade"),
    _edge("W19-S07-R02", "agent/interfaces/cli/manifest.py", "<module>", REMOVE, "agent.actions.* + agent.interfaces.cli.action_registry", "repository CLI callers migrated to the canonical action catalog/projection", "none", "the historical manifest duplicated action metadata ownership", "already absent; user aliases remain in the canonical CLI projection"),
    _edge("W19-S07-R03", "agent/interfaces/cli/query_plane.py", "<module>", REMOVE, "agent.application_services.queries", "repository query callers migrated to the neutral application service", "none", "the CLI facade duplicated the canonical query contracts and service surface", "already absent; no replacement facade"),
    _edge("W19-S07-R04", "agent/interfaces/cli/query_find.py", "<module>", REMOVE, "agent.application_services.query_find", "repository query callers migrated to the neutral FIND helper", "none", "the CLI FIND helper duplicated the canonical query implementation", "already absent; no replacement facade"),
    _edge("W19-S07-R05", "agent/interfaces/cli/query_git.py", "<module>", REMOVE, "agent.application_services.query_git", "repository query callers migrated to the neutral Git helper", "none", "the CLI Git helper duplicated the canonical bounded read-only Git implementation", "already absent; no replacement facade"),
    _edge("W19-S07-R06", "agent/interfaces/cli/app.py", "obter_status_think", REMOVE, "agent.interfaces.cli.thinking_presets", "no repository product caller remains", "none", "the status helper was dead after thinking labels converged on the shared preset owner", "already absent; renderer consumes the shared preset owner"),
    _edge("W19-S07-R07", "agent/interfaces/cli/interactive_commands.py", "show_events", REMOVE, "agent.interfaces.cli.interactive_commands.timeline", "repository interactive callers use the run.timeline action", "none", "the event command duplicated the timeline handler", "already absent; /events remains an alias of run.timeline"),
    _edge("W19-S07-R08", "agent/interfaces/cli/interactive_commands.py", "git_status", REMOVE, "agent.application_services.queries", "repository query admission uses the canonical git_status query kind", "none", "the message stub was not a query owner and could mask canonical query execution", "already absent; /git status submits through the canonical query path"),
