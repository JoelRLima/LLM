...
def _fake_runs(
    ...
    *,
    ...
    monkeypatch.setattr(campaign, "_run_one", fake_run_with_arm)
...
def test_h2_is_exactly_five_even_when_unanimous(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(campaign, "_run_one", fake_run)
    _, summary = campaign._run_scenario(
        _scenario("H2"),
        policy=RepetitionPolicy(),
        gateway_factory=cast(Any, lambda *_args: None),
        candidate=cast(Mapping[str, str], _CANDIDATE),
...
def _analysis_report(*, h3_mixed: bool = False, runtime_incident: bool = False, identity_drift: bool = False) -> dict[str, Any]:
            ...
            for arm in scenario.arms:
                evidence = {
                    ...
                    "observed_model_identity": {
                        ...
                        "observed_model_ids": ["scripted-evaluation"],
                        "distinct_observed_model_ids": ["scripted-evaluation"],
                        ...
                        "call_identities": [{
                            ...
                            "provider": "scripted-evaluation",
                            "endpoint_identity": "in-process://scripted-evaluation",
                            "declared_model": "scripted-evaluation",
                            "observed_provider_model_id": "scripted-evaluation",
                            "identity_source": "response.provider_metadata",
                        }],
                    },
    ...
    return {
        ...
        "observed_model_identity": {
            ...
            "source": "response.provider_metadata",
            "identity_source": "response.provider_metadata",
            "observed_model_ids": ["scripted-evaluation"] * sum(
...
def test_model_identity_is_stable_and_resume_mismatch_is_rejected() -> None:
    ...
    assert identity["provider"] == "scripted-evaluation"
...
def test_fake_provider_identity_is_bound_to_each_run() -> None:
    ...
    assert run.evidence["model_fingerprint"]["provider"] == "scripted-evaluation"
