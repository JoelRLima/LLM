...
def test_h_series_is_exactly_versioned_h1_to_h19() -> None:
    validate_h_series()
    assert H_SERIES_VERSION == "H-SERIES-V1.5"
...
def test_adversarial_audit_covers_all_h_series_arms_with_sixteen_answers() -> None:
    audit = adversarial_audit(Path(__file__).parents[3])
...
def test_resume_requires_exact_candidate_and_campaign_identity(tmp_path) -> None:
    ...
    config = campaign_config(repo_root, output_dir=tmp_path / "evaluation")
